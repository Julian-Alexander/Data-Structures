class Heap {
  ...
  delete() {
    //this checks if heap is not empty
    if (!this.storage.length) return null;
    //this checks if heap only has one element
    ...
    // basically moves the last element in the heap to the root temporarily
    this.storage[0] = this.storage.pop();
    // uses siftDown on the root element to re-arrange the heap
    this.siftDown(0);
    return max;
  }
